...
import org.usfirst.frc.team4028.robot.subsystemSequences.ShootBallSeq;
//import org.usfirst.frc.team4028.robot.subsystems._And1Defense;
import org.usfirst.frc.team4028.robot.autonRoutines.CrossDefenseAuton;
import org.usfirst.frc.team4028.robot.autonRoutines.DoNothingAuton;
import org.usfirst.frc.team4028.robot.autonRoutines.DriveForwardAuton;
import org.usfirst.frc.team4028.robot.autonRoutines.ZeroAllAxisAuton;
import org.usfirst.frc.team4028.robot.constants.AutonConstants;
import org.usfirst.frc.team4028.robot.subsystems.And1;
import org.usfirst.frc.team4028.robot.subsystems.Chassis;
import org.usfirst.frc.team4028.robot.subsystems.Cupid;
...
import org.usfirst.frc.team4028.robot.subsystems.DynamicCameraServer;
import org.usfirst.frc.team4028.robot.subsystems.Infeed;
import org.usfirst.frc.team4028.robot.subsystems.Infeed.INFEED_TILT_POSITION_REENTRANT;
import org.usfirst.frc.team4028.robot.subsystems.Infeed.INFEED_TILT_POSITION_TARGET;
//import org.usfirst.frc.team4028.robot.subsystems._Kicker;
//import org.usfirst.frc.team4028.robot.subsystems._NavigationSensor;
import org.usfirst.frc.team4028.robot.subsystems.Puma;
import org.usfirst.frc.team4028.robot.subsystems.Shooter;
import org.usfirst.frc.team4028.robot.subsystems.Slider;
import org.usfirst.frc.team4028.robot.subsystems.Turret;
import org.usfirst.frc.team4028.robot.subsystems.Winch;
//import org.usfirst.frc.team4028.robot.subsystems._Slider;
//import org.usfirst.frc.team4028.robot.subsystems._Turret;
//import org.usfirst.frc.team4028.robot.subsystems._Winch;
//import org.usfirst.frc.team4028.subsystemSequences.ShootBall;
//import org.usfirst.frc.team4028.subsystemSequences.ShootBall.SHOOT_BALL_STATE;
...
public class Robot extends IterativeRobot 
{
	...
	private ShootBallSeq _shootBall;

	private DynamicCameraServer _roboRioCameraServer;
	// private NavigationSensor _navXSensor;

	// ===========================================================
	// Define class level instance variables for Auton Rountines
	// ===========================================================
	private DoNothingAuton _doNothingAuton;
	private ZeroAllAxisAuton _zeroAllAxisAuton;
	private DriveForwardAuton _driveForwardAuton;
	private CrossDefenseAuton _crossDefenseAuton;
	...
	public void robotInit() {
		...
		_and1 = new And1(RobotMap.AND1_SERVO_PWM_PORT);

		_chassis = new Chassis(RobotMap.LEFT_DRIVE_MASTER_MTR_TALON_CAN_ADDR,
				RobotMap.LEFT_DRIVE_SLAVE_1_MTR_TALON_CAN_ADDR, RobotMap.LEFT_DRIVE_SLAVE_2_MTR_TALON_CAN_ADDR,
				RobotMap.RIGHT_DRIVE_MASTER_MTR_TALON_CAN_ADDR, RobotMap.RIGHT_DRIVE_SLAVE_1_MTR_TALON_CAN_ADDR,
				RobotMap.RIGHT_DRIVE_SLAVE_2_MTR_TALON_CAN_ADDR, RobotMap.PCM_CAN_ADDR,
				RobotMap.SHIFTER_SOLENOID_EXTEND_PCM_PORT, RobotMap.SHIFTER_SOLENOID_RETRACT_PCM_PORT);

		_puma = new Puma(RobotMap.PCM_CAN_ADDR, RobotMap.PUMA_FRONT_SOLENOID_EXTEND_PCM_PORT,
				RobotMap.PUMA_FRONT_SOLENOID_RETRACT_PCM_PORT, RobotMap.PUMA_BACK_SOLENOID_EXTEND_PCM_PORT,
				RobotMap.PUMA_BACK_SOLENOID_RETRACT_PCM_PORT);

		_infeed = new Infeed(RobotMap.INFEED_TILT_MTR_TALON_CAN_ADDR, RobotMap.INFEED_ACQ_MTR_PWM_PORT,
				RobotMap.BALL_IN_POSITION_LIMIT_SWITCH_DIO_PORT);

		_shooter = new Shooter(RobotMap.SHOOTER_LEFT_MTR_PWM_PORT, RobotMap.SHOOTER_RIGHT_MTR_PWM_PORT);
		...
		_shootBall = new ShootBallSeq(_infeed, _shooter);
		...
		// fyi: the camera name (ex "cam0") can be found through the roborio web
		// interface
		// ===================
	...
	/*
	 *****************************************************************************************************
	 * This function is called 1x each time the robot enters anton mode (setup
	 * the initial robot state for auton mode)
	 *****************************************************************************************************
	 */
	public void autonomousInit() {
		...
		if (!_infeed.IsInfeedTiltAxisZeroedYet()) {
			// zero the infeed tilt axis (fyi this may be the same physical
			// location as the stored position)
		...
		Auton_Drive_Throttle_Percent autonFwdThrottlePercent = _driversStation
				.getAutonFwdDriveThrottlePercentRequested();
		Auton_Drive_Time_In_Secs autonFwdDriveTimeInSecs = _driversStation.getAutonFwdDriveTimeInSecsRequested();
		Auton_Drive_Throttle_Percent autonRevThrottlePercent = _driversStation
				.getAutonRevDriveThrottlePercentRequested();
		...
		// Auton_Cross_Defense_Type autonCrossDefenseType =
		// _driversStation.getAutonCrossDefenseTypeRequested();

		// write out the selected Auton Mode
		DriverStation.reportError("AutonModeRequested: [" + _driversStation.getAutonModeRequested().toString() + "]",
				false);
		...
		case DO_NOTHING:
			_doNothingAuton = new DoNothingAuton();
			break;

		case ZERO_ALL_AXIS:
			_zeroAllAxisAuton = new ZeroAllAxisAuton(_infeed);
			break;

		case DRIVE_FWD:
			_driveForwardAuton = new DriveForwardAuton(_infeed, _chassis, _puma, _shootBall, autonFwdThrottlePercent,
					autonFwdDriveTimeInSecs, autonRevThrottlePercent, autonRevDriveTimeInSecs, autonPumaPosition,
					autonInfeedTiltPosition, autonShootBall);
			break;
		/*
		 * case CROSS_DEFENSE: _crossDefense = new CrossDefense(_infeed,
		 * _chassis, _puma, autonCrossDefenseType); break;
		 */
		}
	}
	...
	public void autonomousPeriodic() {
		...
		case DO_NOTHING:
			_doNothingAuton.Execute();
			break;

		case ZERO_ALL_AXIS:
			_zeroAllAxisAuton.Execute();
			break;
	...
	public void teleopPeriodic() {
		...
		// =====================================
		// Step 1.1: Buzz the Gamepad at the start of teleop to ensure driver
		// and operator have correct controllers
		...
		} else if (_driversStation.getIsOnlyInfeedTiltDeployBtnJustPressed()
				|| _infeed.getIsInfeedTiltDeployStillRunning()) {
			_infeed.MoveInfeedToTargetPosition(INFEED_TILT_POSITION_TARGET.DEPLOY);
		} else if ((_driversStation.getIsOnlyInfeedTiltShootBtnJustPressed() && ((_infeed
				.getInfeedTiltTargetPosition() == Infeed.INFEED_TILT_AXIS_DEPLOYED_POSITION_IN_ROTATIONS)
		...
		} else if (_driversStation.getIsOnlyInfeedTiltChevalBtnJustPressed()
				|| _infeed.getIsInfeedTiltChevalStillRunning()) {
			_infeed.MoveInfeedToTargetPosition(INFEED_TILT_POSITION_TARGET.CHEVAL);
		} else if (_driversStation.getIsOnlyInfeedTiltCrossDefenseBtnJustPressed()
				|| _infeed.getIsInfeedTiltCrossDefenseStillRunning()) {
		...
		if (_infeed.getIsTiltAxisAtHomePosition()) {
			// DriverStation.reportError("Infeed Tilt Axis at home position",
			// false);
		}
		...
		/*
		 * Sequence for shooting: Simultaenously acquire at full for x ms (1/4
		 * sec ?) and move infeed to shooting position (or keep it deployed if
		 * you want ball released slowly) Spin shooter wheels to 100% outwards
		 * Cut Infeed Spin infeed 100% out
		 */
		...
		// Throttle & Turn cmds
		_chassis.RunDriveMotors(_driversStation.getArcadeDriveRawThrottleCmd(),
				_driversStation.getArcadeDriveRawTurnCmd());
		
		if (_driversStation.getAnd1RawAxisCmd() > 0.1)
		{
			_and1.StoreAnd1();
		}
		else if (_driversStation.getAnd1RawAxisCmd() < -0.1)
		{
			_and1.DeployAnd1();
		}
		else
		...
		if(_driversStation.getIsCupidToggleBtnPressed())
		{
			_cupid.ToggleCupidPosition();
		}
	...
	public void disabledPeriodic() {
		// DriverStation.reportWarning("Exiting Periodic Scanning Mode!",
		// false);
	}
}
