...
/* Modified by FRC5881 for Camera-Switching
 * Library version based as of 3/5/2016
 */

// FRC5881 Mod - Cleaned up unused imports
...
public class DynamicCameraServer {
  ...
  private static DynamicCameraServer server;

  public static DynamicCameraServer getInstance() {
    if (server == null) {
      server = new DynamicCameraServer();
    }
    return server;
  }
  ...
  // FRC5881 Mod Start
  private boolean m_autoCaptureRunning;
  // FRC5881 Mod End
  ...
  /**
   * Switches the current camera that is being captured. Attempts to stop the current camera first
   * while waiting a maximum of 1s.
  ...
  public void switchAutomaticCapture(final String cameraName) 
  {
	  ...
	  System.out.println("Looping to wait for cap death");
	  
	  int maxLoops = 50; // About 1s max wait
	  
	  while (m_autoCaptureRunning && maxLoops > 0) {
		  try {
			  Thread.sleep(15);
			  maxLoops--;
	  ...
	  if (m_autoCaptureRunning) {
		  System.out.println("DynamicCameraServer -- Failed to stop image thread in time.");
		  return;
	  }
  ...
  {
	  if(_currentCameraName == RobotMap.INFEED_ARM_CAMERA)
	  {
		  switchAutomaticCapture(RobotMap.SIDE_CAMERA);
	  }
	  else
	  {
		  switchAutomaticCapture(RobotMap.INFEED_ARM_CAMERA);
	  }
  }
  ...
  protected void capture() {
	...
	System.out.println("Started AutoCapture");
	// FRC5881 Mod End
    ...
    // FRC5881 Mod Start
    System.out.println("Ended AutoCapture");
    m_autoCaptureRunning = false;
  }

  public synchronized String getCurrentCameraName() 
  {
