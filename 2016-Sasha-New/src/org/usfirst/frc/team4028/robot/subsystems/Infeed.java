...
public class Infeed 
{
	...
	public static final double INFEED_TILT_AXIS_CROSS_DEFENSE_POSITION_IN_ROTATIONS = -0.052;			// this is approx 90 deg
	public static final double INFEED_TILT_AXIS_SHOOTING_POSITION_IN_ROTATIONS = -0.337;
	public static final double INFEED_TILT_AXIS_DEPLOYED_POSITION_IN_ROTATIONS = -0.480;		// this is approx 0 deg
	public static final double INFEED_TILT_AXIS_CHEVAL_POSITION_IN_ROTATIONS = -0.630;			// this is approx ?? deg
	private static final double INFEED_TILT_AXIS_DOWN_SOFT_LIMIT_IN_ROTATIONS = -0.700;
	
	private static final double INFEED_TILT_AXIS_IN_POSITION_THRESHHOLD_IN_ROTATIONS = 0.05;
	...
	public void MoveInfeedTiltAxisToDeployedPositionReentrant()
	{
		...
		double tiltAxisPositionError = Math.abs(Math.abs(_infeedTiltMtr.getPosition()) - Math.abs(_infeedTiltTargetPosition));
		
		if(tiltAxisPositionError > INFEED_TILT_AXIS_IN_POSITION_THRESHHOLD_IN_ROTATIONS)
		{
			_isInfeedTiltDeployStillRunning = true;
			DriverStation.reportError("Is Infeed Deploy Still Running: TRUE", false);
		}
		else
		{
	...
	public void MoveInfeedTiltAxisToChevalPositionReentrant()
	{
	...
	public void MoveInfeedTiltAxisToShootingPositionReentrant()
	{
		...
		if(tiltAxisPositionError > INFEED_TILT_AXIS_IN_POSITION_THRESHHOLD_IN_ROTATIONS)
		{
			...
			DriverStation.reportError("Position error: " + Double.toString(tiltAxisPositionError), false);
		}
		else
		{
	...
	public void MoveInfeedTiltAxisToCrossDefensePositionReentrant()
	{
		if(!_isInfeedTiltCrossDefenseStillRunning)
		{
			// only do when this method is initially called
			DriverStation.reportError("Cross Defense Infeed Tilt Axis", false);
		}		
	...
	{
		switch(infeedTiltPositionReentrant)
		{
			case ZERO:
				_isInfeedTiltDeployStillRunning = false;	
				_isInfeedTiltChevalStillRunning = false;			
